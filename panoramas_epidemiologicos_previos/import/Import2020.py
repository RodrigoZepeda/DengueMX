...
# Directorio 2017
dir_2020 = "panoramas_epidemiologicos_previos/2020"
...
estados = {"CHIAPAS", "VERACRUZ", "NUEVO LEÓN", "GUERRERO", "MICHOACÁN", "TAMAULIPAS",
           ...
           "CIUDAD DE MÉXICO", "AGUASCALIENTES","YUCATAN","MICHOACAN","SAN  LUIS POTOSI",
           "NUEVO LEON","MEXICO","QUERETARO"}

# Directorio de 2017
fnames = [os.path.join(dir_2020, f) for f in listdir(dir_2020)]
...
for name in fnames:
    ...
    if float(epiweek) > 24:
        table = tabula.read_pdf(name, pages=8, area=(100, 7.48, 600, 1000), pandas_options={'header': None})
    elif float(epiweek) < 24:
        table = tabula.read_pdf(name, pages=7, area=(100, 7.48, 600, 700), pandas_options={'header': None})
    else:
        table = tabula.read_pdf(name, pages=8, area=(100, 7.48, 600, 700), pandas_options={'header': None})

    #Pano_dengue_02_2020
    df = table[0]
    df = df[df[0].str.replace("\*| \*", "", regex=True).isin(estados)]

    #Fix some copying errors
    if float(epiweek) == 51.0:
        df.iloc[:, 7].fillna(df.iloc[:, 8], inplace=True)
        df = df.drop(df.columns[[8]], axis=1)  # df.columns is zero-based pd.Index

    if float(epiweek) == 48.0 or float(epiweek) >= 52.0:
        df.iloc[:, 7].fillna(df.iloc[:, 6], inplace=True)
        df = df.drop(df.columns[[6]], axis=1)  # df.columns is zero-based pd.Index
