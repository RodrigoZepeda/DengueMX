...
# | > List files----
dengue_data <- list.files(path = "datos/", pattern = ".*.zip", full.names = TRUE)
...
# | > Estatus del caso ----
caso   <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
...
# | > PCR ----
pcr   <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
...
# | > Dictamen ----
dictamen   <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
...
# | > Institución ----
institucion   <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
...
# | > Paciente ----
paciente   <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
...
# | > Sexo ----
sexo   <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
                         sheet = "CATÁLOGO SEXO")
dengue <- dengue %>% 
...
municipio  <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
                     sheet = "CATÁLOGO MUNICIPIO")
entidad    <- read_excel("diccionario/Catalogos_Dengue.xlsx", 
...
dengue %>%
  write_excel_csv("processed/dengue_2020_2022_mx.csv") %>%
  write_rds("processed/dengue_2020_2022_mx.rds")
...
dengue_pasado  <- read_csv(c("processed/2017.csv", "processed/2019.csv",
                             "processed/2018.csv", "processed/2020.csv")) %>%
...
fechas <- tibble(fecha = seq(ymd("2016/01/01"), max(dengue$Fecha_Sintomas), by = 1)) %>%
...
dengue_all %>%
  write_excel_csv("processed/dengue_2016_2022_mx.csv") %>%
  write_rds("processed/dengue_2016_2022_mx.rds")
